...
# Script name (change this if you rename a.php)
define('SCRIPT_NAME', 'a.php');
...
define('GLYPE_URL',
	...
	. '://'
	. $_SERVER['HTTP_HOST']
	. preg_replace('#/(?:(?:i/)?[^/]*|' . preg_quote(SCRIPT_NAME) . '.*)$#', '', $_SERVER['PHP_SELF'])
);
...
# Load settings
require GLYPE_ROOT . '/i/settings.php';
...
* without POST to the /i/process.php page.
******************************************************************/
...
function injectionJS() {
	...
	$jsFile = GLYPE_URL . '/i/main.js?'.$CONFIG['version'];
